...
def generate_path(N, M):
    ...
    # your code here
    path_list = [(0, 0)]
    while path_list[-1] != (N-1, M-1):
        x, y = path_list[-1][0], path_list[-1][1]
        if x < N-1 and y == M-1: #     a. If at the bottom row of the maze map, move right.
            if maze[x+1][y] != 1:
                x += 1
                path_list.append((x,y))
        elif x == N-1 and y < M-1:#     b. If at the rightmost column of the maze map, move down.
            if maze[x][y+1] != 1:
                y += 1
                path_list.append((x,y))
        else:
            choose = random.sample([0, 1], 1)#     c. Otherwise, randomly choose to move right or down.
            if choose == 0:
                if maze[x+1][y] != 1:
                    x += 1
                    path_list.append((x,y))
    ...
    for i in range(len(path_list)):#update the path
        j, k = path_list[i][0], path_list[i][1]
        maze[j][k] += 2
...
def add_obstacles(maze, num_obstacles):
    ...
    for i in range(len(maze)):
        for j in range(len(i)):
            pos.append((i, j))
    obs = random.sample([i for i in range(len(pos))], num_obstacles)#隨機選取障礙位置
    for i in range(len(obs)):#設置障礙
            j, k = obs[i][0], obs[i][1]
            maze[j][k] += 1
